...
const AddNewYaowen = () => {
  ...
  const [yaowen, setYaowen] = useState<Yaowen>({
    id: uuidv4(),
    title: "",
  ...
  const handlePublish = async () => {
    try {
      const res = await fetch(`http://localhost:3000/api/addNewYaowen`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(yaowen),
      });
    ...
    >
      ...
      <div
        className={`
          ...
          `}
      >
        <h1 className="font-black text-3xl">发布新要闻</h1>
        <div className="w-full flex flex-col gap-2">
          <label htmlFor="title">标题</label>
          <Input
            id="title"
            onChange={(e) =>
              setYaowen((prev) => ({ ...prev, title: e.target.value }))
            }
          />
        </div>
        <div className="w-full flex flex-col gap-2">
        ...
        <div className="w-full flex justify-around items-center gap-2">
          ...
          <div className="flex-1 flex flex-col gap-2">
            ...
            <Input
              id="zebian"
              onChange={(e) =>
                setYaowen((prev) => ({ ...prev, zebian: e.target.value }))
              }
            />
          </div>
          <div className="flex-1 flex flex-col gap-2">
            ...
            <Input
              id="shenhe"
              onChange={(e) =>
                setYaowen((prev) => ({ ...prev, shenhe: e.target.value }))
              }
            />
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          py-6 text-center text-[#12a1a0] text-xl md:text-2xl lg:text-4xl font-black
            `}
          >
            {yaowen.title ? yaowen.title : "请输入标题"}
          </h1>
          ...
          <div className="w-full flex flex-col justify-around items-center mx-auto text-lg pt-2">
            <div>
              发布日期:{" "}
              {yaowen.release_date ? yaowen.release_date : "请输入发布日期"}
            </div>
            <div>访问次数: 0</div>
            <div>
              作者: {yaowen.author.length > 0 ? yaowen.author : "请输入作者"}
            </div>
            <div>
              字号:[
          ...
          <div
            ...
            `}
          >
            {yaowen.content.length > 0 ? (
              yaowen.content.map((item, index) => {
                if (index === 1 && yaowen.images.length > 1) {
                  return (
                    <div key={index}>
                      <h1 className="indent-8 py-2">{item}</h1>
                      <div
                        className={`
                        grid grid-cols-${yaowen.images.length}
                        w-[80%] gap-2 py-[30px] mx-auto
                      `}
                      >
                        {yaowen.images.map((src, i) => (
                          <img
                            key={i}
                            src={src}
                            alt={`image-${i}`}
          ...
          >
            <h1>
              <strong>责编：</strong>
              {yaowen.zebian ? yaowen.zebian : "请输入责编"}
            </h1>
            <h1>
              <strong>审核：</strong>
              {yaowen.shenhe ? yaowen.shenhe : "请输入审核"}
            </h1>
          </div>
        </div>
      </div>
