...
const Yaowen: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [item, setItem] = useState<YaoWenItem | null>(null);
  const [loading, setLoading] = useState(true);
  const [currentFont, setCurrentFont] = useState("中");
  const { pathname } = useLocation();

  useEffect(() => {
    fetch(`http://localhost:3000/api/yaowen/${id}`)
      .then((res) => res.json())
      .then((data: YaoWenItem) => setItem(data))
      .catch(() => setItem(null))
